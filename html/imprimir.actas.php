...
// creacion de variables limpias
	...
	/* formato linea */ $pdf->SetTextColor(50);$pdf->SetFillColor(240); /* formato linea */
		...
		$pdf->Cell(65,6,number_format(($info['VOTOSVALIDOS']*100)/$info['TOTALVOTOS'],2,'.',',') . '%                     ',1,1,'R',true);
	$data = $datoVotos; 
	...
	$pdf->Cell(60,6,number_format($info['TOTALVOTOS'],0,'.',',') . ' (' . number_format($info['PTOTALVOTOS'],2,'.',',') . '%)',1,1,'C',true);
	/* formato linea */ $pdf->SetTextColor(50);$pdf->SetFillColor(220); /* formato linea */
	$pdf->Cell(135,6,utf8_decode('PARTICIPACIÓN'),1,0,'C',true);
	$pdf->Cell(60,6,'ABSTENCIONISMO',1,1,'C',true);
	/* formato linea */ $pdf->SetTextColor(50);$pdf->SetFillColor(240); /* formato linea */
	$pdf->Cell(135,6,number_format(($info['CNTVOTANTES']-$info['ABSTENCIONISMO']),0,'.',',') . ' (' . number_format((($info['CNTVOTANTES']-$info['ABSTENCIONISMO'])*100)/$info['CNTVOTANTES'],2,'.',',') . '%)',1,0,'C',true);
	$pdf->Cell(60,6,number_format($info['ABSTENCIONISMO'],0,'.',',') . ' (' . number_format(($info['ABSTENCIONISMO']*100)/$info['CNTVOTANTES'],2,'.',',') . '%)',1,1,'C',true);
	$pdf->ln(2);
	/* formato linea */ $pdf->SetTextColor(50);$pdf->SetFillColor(220); /* formato linea */
	$pdf->Cell(65,5,'VOTANTES INSCRITOS',1,0,'C',true);
	$pdf->Cell(65,5,'JRV TOTALIZADAS',1,0,'C',true);
	$pdf->Cell(65,5,'TOTAL JRV',1,1,'C',true);
	/* formato linea */ $pdf->SetTextColor(50);$pdf->SetFillColor(240); /* formato linea */
	$pdf->Cell(65,6,number_format($info['CNTVOTANTES'],0,'.',','),1,0,'C',true);
	$pdf->Cell(65,6,number_format($info['MESASPRO'],0,'.',',') . ' (' . number_format($info['PMESASPRO'],2,'.',',') . '%)',1,0,'C',true);
	$pdf->Cell(65,6,number_format($info['CNTMESAS'],0,'.',','),1,1,'C',true);
	$pdf->ln(6);	
