...
$titulo = 'Repetición de la Elección de Corporación Municipal de Nebaj, Quiché';
...
define('FPDF_FONTPATH','../../include_nebaj/fpdf/font/');
require('../../include_nebaj/phplot/phplot.php');
require('../../include_nebaj/fpdf/mem_image.php');
...
//	sort($data);
	...
	/* formato linea */ $pdf->SetTextColor(50);$pdf->SetFillColor(240); /* formato linea */
		...
		if ($info['PTOTALVOTOS']>0)
		$pdf->Cell(65,6,number_format(100,2,'.',',') . '%                     ',1,1,'R',true);
		else
		$pdf->Cell(65,6,number_format(0,2,'.',',') . '%                     ',1,1,'R',true);
	...
	/* formato linea */ $pdf->SetTextColor(50);$pdf->SetFillColor(240); /* formato linea */
	$pdf->Cell(65,6,number_format($info['TOTALVOTOS'],0,'.',',') . ' (' . number_format((($info['TOTALVOTOS'])*100)/$info['CNTVOTANTES'],2,'.',',') . '%)',1,0,'C',true);
	$pdf->Cell(65,6,number_format(($info['CNTVOTANTES']-$info['TOTALVOTOS']),0,'.',',') . ' (' . number_format((($info['CNTVOTANTES']-$info['TOTALVOTOS'])*100)/$info['CNTVOTANTES'],2,'.',',') . '%)',1,0,'C',true);
	...
	$pdf->Cell(97,6,number_format($info['CNTMESAS'],0,'.',',') . ' (100.00%)',1,1,'C',true);
	$pdf->ln(6);	
	
	}

/* inicio define nombre de documento */
$subtitulo = strtolower('Repetición de la Elección de Corporación Municipal de Nebaj Quiché');
