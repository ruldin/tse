...
// Clase para  despliegue de gráficas
	...
	function conectarDB() {
       ...
       $this->ors=$cdb->consulta("SELECT * FROM tresultado WHERE DEP=".$deptoProceso." and MUN=".$munProceso." and TIPOELECCION=".$teProceso);
	}
    ...
    function barras() {
       ...
       if ($this->ors > 0 ){
            ...
            while ( $i< $this->f_tp && $this->ors[11 +($i*5)]<>'' ) {
     		  $bar = new bar_value($this->ors[13+($i*5)]);
			  if($this->ors[13+($i*5)]>$mayor) $mayor=$this->ors[13+($i*5)];
              $bar->set_tooltip($this->ors[11+($i*5)].'<br>#val#<br>'.number_format($this->ors[14+($i*5)],2).'%');
              $bar->set_colour( $this->ors[12+($i*5)] );
     		  $data[]  = $bar;
              $dataN[] = $this->ors[11+($i*5)];		  
    		  $i++;
            }
			$otros=0;$otrospor=0;$haymas=0;
            while ($i<$this->f_mp && $this->ors[11+($i*5)]<>'' ) {
			  $otros= $otros+$this->ors[13+($i*5)];
			  $otrospor=$otrospor+ number_format($this->ors[14+($i*5)],2);
			  $i++;
			  $haymas=1;
			}
       ...
       $x_axis->set_labels_from_array( $dataN );

       $y_axis = new y_axis();
       $y_axis->set_range( 0, $this->ceiling($mayor,100), round($mayor/10,-1) ); 
...
//	   $this->ofigura=$chart->toPrettyString();
    ...
    function pastel() {
       ...
       if ($this->ors > 0 ){
        	$i=0;
            while ( $i< $this->f_tp && $this->ors[11 +($i*5)]<>'' ) {
     	   	   $tmp = new pie_value($this->ors[13+($i*5)]/1,$this->ors[11+($i*5)]);
			   $tmp->set_tooltip($this->ors[11+($i*5)].'<br>#val#<br>'.number_format($this->ors[14+($i*5)],2).'%');
               $tmp->set_colour( $this->ors[12+($i*5)] );
               $data[] = $tmp;
       		   $i++;
           }
			$otros=0;$otrospor=0;$haymas=0;
