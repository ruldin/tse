...
// Clase para  despliegue de gráfica de Barras
    ...
    function nombremunicipio($depto) {
        $amuni = array ( 1 => array ( 0 => 'Departamental',
		                              1 => 'Distrito Central', 2 => 'Santa Catarina Pinula', 3 => 'San Jos&eacute; Pinula', 
...
class clugar {
   ...
   function listalugar() {
	...
	if ($this->p_te==1)
	{
		...
		elseif ($this->p_depto!=0 && $this->p_muni==0)
		{
		...
		if ($this->p_inicio ==1) $this->alugar[0]='Seleccione';
		$i=0;
		while (list($i,$valor)=each($this->alugar))
		$lista=$lista .'<option value="'.$i.'">'.$valor.'</option>';
		$lista = $lista.'</select>';
		return $lista;
	}
}
	...
	function combolugarmesa() {
		...
		if ($this->p_te==1)
		{
			...
			$this->alugar[0]='Seleccione';
		}
		elseif ($this->p_te==4)
		{
			if($this->p_depto==0 && $this->p_muni==0) {
				$this->alugar = $eleccion->nombredepartamentoAlcaldes();
				$lista='Departamento:<br/><select name="dep" onChange="'.$this->p_nomfun.'(document.formesa.dep.options[document.formesa.dep.selectedIndex].value)">';
				$lista.='<option value="0">Seleccione</option>';
			}
			elseif($this->p_depto!=0 && $this->p_muni==0) {
			   $this->alugar = $eleccion->nombremunicipioAlcaldes($this->p_depto);
			   $lista='Municipio:<br/><select name="mun" onChange="'.$this->p_nomfun.'(document.formesa.mun.options[document.formesa.mun.selectedIndex].value)">';
			}
			else {			
			   $this->alugar = $eleccion->nombremunicipioAlcaldes($this->p_depto);
