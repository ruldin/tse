...
// Clase para  despliegue de gráfica de Barras
   ...
   function construyeTabla($dep,$mun,$tipoeleccion,$titulo) {
	...
	if(version_compare(phpversion(),"4.3.0", ">")){
	   ...
	   $i=0;
	   echo '<thead><tr><th class="header1">Partidos</th><th class="header2" style="font-size:10px;">Votos v&aacute;lidos</th><th class="header3">%</th></tr></thead><tbody>';
       $_SESSION['HD']=0;
	   ...
	   echo "<table class=\"tabla\">";
	   echo "<thead><tr><th class='header1'>Votos</th><th class='header2'>Totales</th><th class='header3'>%</th></tr></thead><tbody>";
	   echo "<tr class=\"totimpar\"><td>V&aacute;lidos:</td><td>".number_format($this->ors['VOTOSVALIDOS'],0)."</td><td></td></tr>" ;
	   echo "<tr class='odd'><td>Nulos:</td><td>".number_format($this->ors['NULOS'],0)."</td><td>".number_format($this->ors['PNULOS'],2)."</td></tr>" ;
	   echo "<tr class=\"totimpar\"><td>Blancos:</td><td>".number_format($this->ors['BLANCOS'],0)."</td><td>".number_format($this->ors['PBLANCOS'],2)."</td></tr>" ;           $pemitidos=$this->ors['NULOS']+$this->ors['BLANCOS']+$this->ors['VOTOSVALIDOS'];
           $pemitidos=$pemitidos*100;
           if($this->ors['CNTVOTANTES']>0) $pemitidos=$pemitidos/$this->ors['CNTVOTANTES'];
	   echo "<tr class='odd'><td>Emitidos:</td><td>".number_format($this->ors['NULOS']+$this->ors['BLANCOS']+$this->ors['VOTOSVALIDOS'])."</td><td>".number_format($pemitidos,2)."</td></tr>" ;
	echo "</tbody></table><br />";
	echo "<table class='tabla2'><caption>Datos de Padr&oacute;n</caption><tbody>";
   echo "<tr class=\"totimpar\"><td class=\"c1\">Votantes Inscritos:</td><td class=\"c2\">".number_format($this->ors['CNTVOTANTES'],0)."</td><td class=\"c3\"></td></tr>" ;
	   echo "<tr class='odd'><td class='c1' title=\"Juntas Receptoras de Votos Totalizadas\">JRV Totalizadas:</td><td class=\"c2\">".number_format($this->ors['MESASPRO'],0)."</td><td class=\"c3\">".number_format($this->ors['PMESASPRO'],2)."</td></tr>" ;
	   echo "<tr class='totimpar'><td class='c1' title=\"Total Juntas Receptoras de Votos\">Total JRV:</td><td class=\"c2\">".number_format($this->ors['CNTMESAS'],0)."</td><td class=\"c3\"></td></tr>" ;
	   echo "</tbody></table>";
   }
   ...
   function construyeListado($dep,$mun,$tipoeleccion,$titulo) {
	...
	if(version_compare(phpversion(),"4.3.0", ">")){
	   ...
	   $html.='<tr '.$tclass.'><td><a href="#" onClick="javascript:Cargar_mesa('.$this->ors['NROMESA'].','.$tipoeleccion.')">'.$this->ors['NROMESA'].'</a></td>';
   ...
   function construyeTablaMesa($tmesa,$tipoeleccion,$detalle) {
       $cdb = new cbase;
       $cdb->conectaDB();
	if(version_compare(phpversion(),"4.3.0", ">")){
	   ...
	   $html='<div id="diacta"><br/>';
	   if ($detalle==1)
	   $html.='<a href="#" onClick="javascript:cargar_objeto(3,0)">&laquo;&laquo; Volver</a><br/>';

		//Validación de imagen con extensión JPG o PNG
	   if (file_exists(DIRECTORIO_ACTAS.str_pad((int)$tmesa,5,"0",STR_PAD_LEFT).$tipoeleccion.'.'.EXTENSION_ACTAS))
	   $html.='<a href="'.DIRECTORIO_ACTAS.str_pad((int)$tmesa,5,"0",STR_PAD_LEFT).$tipoeleccion.'.'.EXTENSION_ACTAS.'" target="_blank"><img src="'.DIRECTORIO_ACTAS.str_pad((int)$tmesa,5,"0",STR_PAD_LEFT).$tipoeleccion.'.'.EXTENSION_ACTAS.'" /></a></div>';
	   elseif (file_exists(DIRECTORIO_ACTAS.str_pad((int)$tmesa,5,"0",STR_PAD_LEFT).$tipoeleccion.'.'.EXTENSION_ACTAS_PNG))
	   $html.='<a href="'.DIRECTORIO_ACTAS.str_pad((int)$tmesa,5,"0",STR_PAD_LEFT).$tipoeleccion.'.'.EXTENSION_ACTAS_PNG.'" target="_blank"><img src="'.DIRECTORIO_ACTAS.str_pad((int)$tmesa,5,"0",STR_PAD_LEFT).$tipoeleccion.'.'.EXTENSION_ACTAS.'" /></a></div>';
	   else
	   $html.='<a href="'.DIRECTORIO_ACTAS.'nodisponible.png" target="_blank"><img src="'.DIRECTORIO_ACTAS.'nodisponible.png" /></a></div>';
	   ...
	   $html.='<tr class="totimpar"><td class="c1">Votos Nulos:</td><td class="c2">'.$this->ors['NULOS'].'</td><td class="c3\"></td></tr>';
	   $html.='<tr class="totpar"><td class="c1">Votos Blancos:</td><td class="c2">'.$this->ors['BLANCOS'].'</td><td class="c3\"></td></tr>';
	   $html.='<tr class="totimpar"><td class="c1">Votos Emitidos:</td><td class="c2">'.($this->ors['TOTALACTA']).'</td><td class="c3"></td></tr>';
	   $html.='<tr class="totpar"><td class="c1">Papeletas Recibidas:</td><td class="c2">'.$this->ors['CNTPAPELETAS'].'</td><td class="c3"></td></tr>';
	   $html.='<tr class="totimpar"><td class="c1">Votantes Inscritos:</td><td class="c2">'.number_format($this->ors['CNTINSCRITOS'],0).'</td><td class="c3"></td></tr>';
	   $html.='</tbody></table></div>';
